$("#queryCategoryLevel1").change(function(){
	...
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			...
			url:"/AppInfo/dev/flatform/app/categorylevellist.json",
...
$("#queryCategoryLevel2").change(function(){
	...
	if(queryCategoryLevel2 != '' && queryCategoryLevel2 != null){
		$.ajax({
			type:"GET",//请求类型
			url:"/AppInfo/dev/flatform/app/categorylevellist.json",//请求的url
...
function setSelectName() {
    var index = $('#typeCode option:selected').text();
    var index1=$('#typeCode option:selected').val();
    var optionJson=[];
    $.ajax({
        url: "selectTypeName.json",
        type: "post",
        data: {"typeCode":index1},
        datatype: "json",
        success:function(data){
            alert(data.length);
//            alert(data[0].college);
            optionJson = data;
            var a=data.length;
            var selectObj=document.getElementById("typeName");
            selectObj.options.length=0;
            selectObj.options.add(new Option("请选择类型名称",0));
            for(var i=0;i<data.length;i++){
                selectObj.add(new Option(optionJson[i].typeName,i+1));
            }
        }
    })
}
